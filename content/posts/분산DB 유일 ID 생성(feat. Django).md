...
**계좌와 거래내역 레코드를 시스템 내에서 식별할 수 있는 유일 ID가 필요**하다. 시스템 전체에서 데이터를 통합해서 봐야하는 경우가 있기 때문이다. 전체 데이터를 조회해야할 수도 있고, DB 증설 등으로 인해 데이터를 다른 DB로 옮겨야할 일이 종종 발생한다. 자동 생성되는 `id` 필드는 각 DB 내에서만 유일함을 보장받을 뿐이다. 
